...
{
public:
	LocationVector(double X, double Y, double Z, double O) : x(x), y(Y), z(Z), rot(O) {}
...
public:
	...
	bool toZero32PaddedFloatBuf(ByteBuffer &outputBuf) const
	{
		outputBuf << float(0) << float(x)<< float(0) << float(y) << float(0) << float(z);
		return true;
	}
	bool toZero32PaddedFloatBuf(byte *outputBuf,size_t maxLen) const
	{
		ByteBuffer tempByteBuf;
		toZero32PaddedFloatBuf(tempByteBuf);
