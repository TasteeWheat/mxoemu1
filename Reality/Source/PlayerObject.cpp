...
#pragma region firstByte == 0x29
		...
		if (secondByte == 0x07) //whisper
		{
			...
			string serverPrefix = "SOE+MXO+Recursion+";
...
#pragma endregion
	...
	{
#pragma region (firstByte == 0x30) //perform emote
		uint32 emoteId;
		uint32 emoteTarget;
...
#pragma region (firstByte == 0x81)
		...
		{
#pragma region (secondByte == 0x54) //whereami
...
#pragma endregion
		...
		{
#pragma region (secondByte == 0x92) //get player details
			uint32 zeroInt;
...
#pragma endregion
		...
		{
#pragma region (secondByte == 0x96) //set background
...
#pragma region AddHardlineTheyAreUsing if it isnt in the DB
			double X,Y,Z,O;
			X = this->getPosition().x;
			Y = this->getPosition().y;
			Z = this->getPosition().z;
