...
void GameClient::SendEncrypted(SequencedPacket withSequences)
{
	...
	DEBUG_LOG( format("SENDING %1%") % Bin2Hex(withSequences.getDataWithHeader()) );
	if (Bin2Str(withSequences.getDataWithHeader()).find("resource"))
	{
	int j = 0;
	}
	DEBUG_LOG( format("SENDING %1%") % Bin2Str(withSequences.getDataWithHeader()) );
